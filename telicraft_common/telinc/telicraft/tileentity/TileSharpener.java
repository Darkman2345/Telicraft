...
public class TileSharpener extends TileEntity implements IInventory,
		ISidedInventory {
	...
	/** The number of ticks that the Sharpener will keep active */
	public int sharpenerSharpTime = 0;

	/**
	 * The number of ticks that a fresh copy of the currently-sharpening item would
	 * keep the Sharpener burning for
	 */
	public int currentItemSharpTime = 0;

	/** The number of ticks that the current item has been sharpening for */
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			if (this.sharpenerSharpTime == 0 && this.canSharpen()) {
				this.currentItemSharpTime = this.sharpenerSharpTime = getItemSharpness(this.sharpenerItemStacks[1]);
			...
			if (this.isBurning() && this.canSharpen()) {
	...
	/**
	 * Returns true if the Sharpener can sharpen an item, i.e. has a source item,
	...
	/**
	 * Turn one item from the Sharpener source stack into the appropriate smelted
	 * item in the Sharpener result stack
	 */
	 ...
	 * Sharpener active, or 0 if the item isn't applicable.
	 */
	public static int getItemSharpness(ItemStack par0ItemStack) {
	...
	/**
	 * Return true if item is sharp (getItemBurnTimeSharpness() > 0).
	 */
	public static boolean isItemSharp(ItemStack par0ItemStack) {
		return getItemSharpness(par0ItemStack) > 0;
	}
