...
public class TileEntityAdamantFurnace extends TileEntity implements IInventory,
		ISidedInventory {
	 ...
	 */
	public static boolean heatIncrease = false;
	
	/**
	 * If true, the furnace will start increasing the heat next time furnaceCookTime
	 * is 0.
	 */
	...
	public void readFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		this.shouldStartIncreasing = par1NBTTagCompound.getBoolean("ShouldIncrease");
	}
	...
	public void writeToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		par1NBTTagCompound.setBoolean("ShouldIncrease", this.shouldStartIncreasing);
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (furnaceItemStacks[0] == null && furnaceItemStacks[3] != null) {
				...
				if(furnaceItemStacks[0] != null && furnaceItemStacks[3] != null){
					...
					if(var3 == var4){
						int var5 = furnaceItemStacks[0].stackSize + furnaceItemStacks[3].stackSize;
						
						if(var5 > 64){
							int var6 = 64 - furnaceItemStacks[0].stackSize;
							
							furnaceItemStacks[0].stackSize = 64;
							furnaceItemStacks[3].stackSize = furnaceItemStacks[3].stackSize - var6;
		...
		if(this.shouldStartIncreasing){
			if(this.furnaceCookTime == 0){
				this.shouldStartIncreasing = false;
		...
		if(this.heat > 499){
			...
			this.worldObj.createExplosion((Entity)null, this.xCoord, this.yCoord, this.zCoord, MainReferences.MACHINE_EXPLOSION_STRENGTH, true);
		}
	...
	public static void setHeatIncrease(boolean par1){
		if(par1){
			shouldStartIncreasing = true;
		}else{
			heatIncrease = false;
			shouldStartIncreasing = false;
		}
	}
	...
	public int getStartInventorySide(ForgeDirection par1Side) {
		if(par1Side == ForgeDirection.WEST){
			...
			return 2;
		}else if (par1Side == ForgeDirection.NORTH || par1Side == ForgeDirection.UP){
			return 3;
		}
